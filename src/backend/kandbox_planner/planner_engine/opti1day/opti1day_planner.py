...
from kandbox_planner.util.travel_time  import  HaversineTravelTime as TravelTime

class Opti1DayPlanner:
    working_dir = None 
    planner_code = 'opti1day'
    log_search_progress = True
    nbr_days_in_batch = 1 # for future
    max_exec_time = 20

    def __init__(self, max_exec_time=None ):       
        self.kplanner_db = KPlannerDBAdapter()
        self.travel_router = TravelTime(travel_speed=20)
        if max_exec_time is not None:
            self.max_exec_time = max_exec_time

    def ____connect_TODEL(self):
        pass

    def _get_travel_time_2_sites(self, site1, site2):
        #y_1,x_1= site1.split(':')
        #y_2,x_2= site2.split(':')
        new_time = self.travel_router.get_travel_minutes_2locations(  
            [ site1['geo_longitude'],site1['geo_latitude'] ],  
            [ site2['geo_longitude'],site2['geo_latitude'] ] )
        if new_time > 200:
            print([ site1['geo_longitude'],site1['geo_latitude'] ],  
                [ site2['geo_longitude'],site2['geo_latitude'] ] , 
                (new_time))
        # print("travel: ", new_time)
        return int(new_time/1)

    def dispatch_jobs(self, start_date = '20191101', end_date =  '20191230'):
        
        GENERATOR_START_DATE = datetime.strptime(start_date, config.KANDBOX_DATE_FORMAT)
        GENERATOR_END_DATE =  datetime.strptime(end_date, config.KANDBOX_DATE_FORMAT)
        ...
        while  current_date < GENERATOR_END_DATE: 
            ...
            self.kplanner_db.purge_planner_job_status(planner_code=self.planner_code,  start_date = current_start_day, end_date =  date_util.add_days_2_day_string(current_start_day, 1) )

            workers,w_dict = self.kplanner_db.load_transformed_workers( start_day = current_start_day ) 
            jobs_orig = self.kplanner_db.load_jobs_original_df(  start_day = current_start_day, end_day =  date_util.add_days_2_day_string(current_start_day, 1))
            if len(jobs_orig) < 1:
                print("it is empty, nothing to dispatch!")
                return

            jobs_orig['requested_worker_code'].fillna('SHOULD_NOT_EXIST', inplace=True)
            w_dict['SHOULD_NOT_EXIST'] = 0
            jobs_orig['requested_worker_index'] = jobs_orig.apply(lambda x:  
                                                            w_dict[ x['requested_worker_code'] ] 
                                                            , axis=1 )
            jobs_orig['requested_duration_minutes'] = jobs_orig.apply(lambda x:  
                                                            int(  x['requested_duration_minutes'] )
                                                            , axis=1 )
            '''
            current_shifts=[]
            ...
            '''       
            current_shifts = list(jobs_orig.T.to_dict().values())
            current_workers = workers#.T.to_dict().values()

            print({'loaded day':current_date, "job count": len(current_shifts),  "current_workers count": len(current_workers) })  
            worker_day = self.dispatch_jobs_1day(jobs = current_shifts, workers = current_workers ) # [0:20] [0:133]
            if len(worker_day) < 1:
            ...
            for w_i in range(len(worker_day)):
                ...
                for task in worker_day[w_i][:-1]:
                    ...
                    job_list.append ({  
                        ...
                        "job_type": current_shifts[task_id]['job_type'], 
                        "planning_status": "I", 
                        "scheduled_worker_code": worker_code,
                        "scheduled_start_day": datetime.strftime(current_date,config.KANDBOX_DATE_FORMAT),
                        "requested_start_day":  datetime.strftime(current_date,config.KANDBOX_DATE_FORMAT),
                        "scheduled_start_minutes": task[1]  ,
                        "scheduled_duration_minutes": current_shifts[task_id]['requested_duration_minutes']  ,  
                        ...
                        "geo_longitude": current_shifts[task_id]['geo_longitude']  ,
                        "geo_latitude": current_shifts[task_id]['geo_latitude']  ,
                        "conflict_level": 0,
            ...
            # job_df = self.calc_travel_minutes_before_after_df(job_df)

            curr_game_code = '{}-{}'.format(self.planner_code, current_start_day)  # datetime.strftime(current_date,config.KANDBOX_DATE_FORMAT) 
            game_info = {
                'planner_code': self.planner_code,
                'game_code': curr_game_code,
            }

            #job_df['planner_code'] = self.planner_code
            #job_df['effective_from_date'] = datetime.now()
            #job_df['game_code'] = curr_game_code

            jobs = self.kplanner_db._convert_df_day_minutes_to_datetime(jobs_df= job_df, drop_original=False)
            self.kplanner_db.save_schedued_jobs(jobs,game_info)
            self.kplanner_db.save_game_info(game_code=curr_game_code
                , planner_code= self.planner_code
                , data_start_day = datetime.strftime(current_date, config.KANDBOX_DATE_FORMAT)
                , data_end_day = datetime.strftime( current_date + timedelta(days=1) , config.KANDBOX_DATE_FORMAT))
    ...
    def calc_travel_minutes_before_after_df(self,df):
        ...
        visit_cust_I = df[df['planning_status'] !='U'].sort_values(['scheduled_worker_code', 'scheduled_start_day', 'scheduled_start_minutes'], ascending=True).copy()

        visit_cust_I[['prev_job_code','prev_geo_longitude','prev_geo_latitude'
                ,'prev_start_day','prev_start_minute' 
                ,'prev_duration_minutes','prev_worker_code','prev_location_code']]  \
        =   visit_cust_I[['job_code',  'geo_longitude', 'geo_latitude' 
                , 'scheduled_start_day', 'scheduled_start_minutes' 
            , 'scheduled_duration_minutes','scheduled_worker_code', 'location_code']].shift(periods=1, fill_value=999999).apply(lambda x: x, axis=1, result_type='expand')

        visit_cust_I[[ 'next_start_day', 'next_worker_code']]  \
        =   visit_cust_I[[  'scheduled_start_day', 'scheduled_worker_code' ]].shift(periods=-1, fill_value=999999).apply(lambda x: x, axis=1, result_type='expand')


        visit_cust_U = df[df['planning_status'] =='U'].sort_values(['scheduled_worker_code', 'scheduled_start_day', 'scheduled_start_minutes'], ascending=True).copy()
        visit_cust_U[['prev_job_code','prev_geo_longitude','prev_geo_latitude'
                ,'prev_start_day','prev_start_minute' 
                ,'prev_duration_minutes','prev_worker_code','prev_location_code'
                ,'next_start_day', 'next_worker_code']] = visit_cust_U.apply(lambda x: ['_NONE',0,0,0,0,0,'_NO_WORKER','_NO_LOC',0,'_NO_WORKER'] , axis=1, result_type='expand' )

        visit_cust=pd.concat([visit_cust_I, visit_cust_U])


        visit_cust[['scheduled_travel_minutes_before','scheduled_travel_minutes_after','scheduled_travel_prev_code', 'scheduled_travel_next_code']] \
            = visit_cust.apply(lambda x: self.calc_travel_minutes_before_after(x) , axis=1, result_type='expand' )
    ...
    def calc_travel_minutes_before_after_func(self,x):
        ...
        if x['scheduled_worker_code'] != x['prev_worker_code']:
            from_home = True 
        if x['scheduled_start_day'] != x['prev_start_day']:
            from_home = True 
            
        if x['scheduled_worker_code'] != x['next_worker_code']:
            to_home = True 
        ...
        #    return fromjob, 0
        try:
            home_travel_time =self.travel_router.get_travel_minutes_2locations([float(x['geo_longitude']),float(x['geo_latitude'])    ],\
                                                                            [float(x['w_geo_longitude']),float(x['w_geo_latitude']) ] )  
            prev_job_travel_time =self.travel_router.get_travel_minutes_2locations([float(x['geo_longitude']),float(x['geo_latitude'])    ],\
    ...
    def dispatch_jobs_1day(self, workers = [], jobs = [] ):
        ...
        # Computed data.
        all_jobs_total_working_time = sum([ jobs[j]['requested_duration_minutes'] for j in range(1, num_jobs) ])

        random_total_travel_time = sum( [self._get_travel_time_2_sites(jobs[shift-1], jobs[shift])    for shift in range(1, num_jobs)   ] )
        ...
        workers_assigned2_job_literals = collections.defaultdict(list)   # emp_job_literals [job] = [worker_n_lit, ....]
        ...
        source_lit_dict = {}
        sink_lit_dict = {}
        total_travel_until_emp[-1] = model.NewIntVar(0, 0 , 'total_travel until emp {}'.format('init')) 

        for ei in range(num_workers):
            source_lit_dict[ei] = {} 
            sink_lit_dict[ei] = {} 
            travel_time_per_emp_sum_dict[ei] = model.NewIntVar(0, MAX_MINUTE , 'total_travel time for emp {}'.format(ei)) 
            total_travel_until_emp[ei] = model.NewIntVar(0, MAX_MINUTE * (ei + 1) , 'total_travel for emp {}'.format(ei)) 

            # To chain and  accumulate all travel times for each employee 
            model.Add(total_travel_until_emp[ei]  == total_travel_until_emp[ei-1] + travel_time_per_emp_sum_dict[ei]    )
        # total traval (travel_time_final_total) is the last one
        travel_time_final_total =  model.NewIntVar(0, num_workers * int(MAX_MINUTE * 1) , 'total_travel for  - {}'.format('all')) 
        ...
        for shift in range(num_jobs):
            # 
            if  jobs[shift]['mandatory_minutes_minmax_flag'] == 1: 
                shift_start_time_dict[shift] =  model.NewIntVar(jobs[shift]['requested_start_min_minutes'], jobs[shift]['requested_start_max_minutes'] , 'start_time_shift_%i' % shift)
            else:
                shift_start_time_dict[shift] =  model.NewIntVar(1, MAX_MINUTE, 'start_time_shift_%i' % shift)

            #if jobs[shift]['job_type'] == 'FS':
            #    model.Add( shift_start_time_dict[shift] ==  jobs[shift]['requested_start_minutes'] ) 

            travel_time_until_job[shift] =  model.NewIntVar(1, MAX_MINUTE, 'travel_time_until_shift_%i' % shift)
            outgoing_other_job_index.append ( [] )
            for ei in range(num_workers):

                source_lit_dict[ei][shift] = model.NewBoolVar('from emp {} source to job {}'.format(ei, shift))
                ...
                model.Add(travel_time_until_job[shift] == self._get_travel_time_2_sites(workers[ei], jobs[shift])
                    ).OnlyEnforceIf( source_lit_dict[ei][shift])

                sink_lit_dict[ei][shift] = model.NewBoolVar('from  job {} sink to emp {} '.format( shift,ei))
                ...
                model.Add( travel_time_per_emp_sum_dict[ei] ==  travel_time_until_job[shift] + self._get_travel_time_2_sites(workers[ei], jobs[shift])
                    ).OnlyEnforceIf( sink_lit_dict[ei][shift])  # from sink_    

                emp_on_job_lit = model.NewBoolVar('path Emp {} on job {}'.format(ei, shift)) 
                workers_assigned2_job_literals[shift].append(emp_on_job_lit)  # ordered by worker_index
        
                # job must obey Start time for worker, if assigned to this worker
                # TODO, only 1 day for now, [0]
                model.Add( shift_start_time_dict[shift]  >= workers[ei]['working_minutes'][0][0]
                    ).OnlyEnforceIf( emp_on_job_lit) 
                # 
                # job must obey end time for worker, if assigned to this worker
                model.Add( shift_start_time_dict[shift]  <= int( workers[ei]['working_minutes'][0][1] - jobs[shift]['requested_duration_minutes'])
                    ).OnlyEnforceIf( emp_on_job_lit)  

                model.Add( emp_on_job_lit ==  1 ) .OnlyEnforceIf( sink_lit_dict[ei][shift])  # from sink_    
                model.Add( emp_on_job_lit ==  1 ) .OnlyEnforceIf( source_lit_dict[ei][shift])  # from sink_    
        ...
        for shift in range(num_jobs):
            ...
            for other in range(num_jobs):
                if ( shift == other ) :
                    continue
                '''
                if ( jobs[shift]['mandatory_minutes_minmax_flag']  == 1 ) & ( jobs[other]['mandatory_minutes_minmax_flag']  == 1 ) & \
                   ( jobs[other]['requested_start_min_minutes'] - jobs[shift]['requested_start_max_minutes'] < min_delay_between_jobs):
                    continue
                '''
                ...
                model.Add( shift_start_time_dict[shift] + jobs[shift]['requested_duration_minutes'] + min_delay_between_jobs + self._get_travel_time_2_sites(jobs[shift] , jobs[other] )  \
                        <  shift_start_time_dict[other] ).OnlyEnforceIf(lit)

                # Increase travel time
                model.Add(travel_time_until_job[other] == travel_time_until_job[shift] + self._get_travel_time_2_sites(jobs[shift] , jobs[other] ) ).OnlyEnforceIf(lit)


                # Make sure that for one flow path, all jobs belong to one worker 2020-01-07 12:59:21
                for ei in range(num_workers):
                    model.Add(workers_assigned2_job_literals[other][ei] == workers_assigned2_job_literals[shift][ei]
                        ).OnlyEnforceIf(lit)
        ...
        solver.parameters.max_time_in_seconds = self.max_exec_time # two minutes
        ...
        for ei in emp_following_tasks.keys(): 
            ...
            while my_next_node[0] == 'job':
                ...
                to_print_json_list[ei].append([ 
                    ...
                    self._get_travel_time_2_sites( jobs[int(prev_node[1])]  , jobs[ shift] ) , 
                    jobs[ shift]['requested_duration_minutes'] 
                    ] ) 
                prev_node = my_next_node
                my_next_node = all_following_tasks[shift]
            to_print_json_list[ei].append([-1, -2, self._get_travel_time_2_sites( workers[ei] , jobs[ shift] ) , -4  ] ) 
...
if __name__ == '__main__': 
    ...
    # from kandbox_planner.util.travel_time  import  TaxicabTravelTime as TravelTime
    '''
    opti = Opti1DayPlanner( max_exec_time = 20)
    from kandbox_planner.util.travel_time  import  TaxicabTravelTime 
