...
class RewardFunction:
  ...
  def evalute_normal(self, env=None, job_index_list = []):
    ...
    # return score, violated_rules (negative values)
  '''
  def generate_virtural_job_from_action(self, env=None, a_dict = None, job_i = None):
    if job_i is None:
    ...
    for w_i, worker_code in enumerate(all_workers):
      ...
      job['scheduled_worker_code'] = worker_code
      rest_workers.pop(w_i)
      job['scheduled_related_worker_code'] = rest_workers
      ...
      job['assigned_start_minutes'] = action_day * env.config['minutes_per_day'] + a_dict['scheduled_start_minutes']
  ...
  def evalute_action_normal(self, env=None, action_dict = None, job_i=None):
    ...
    all_jobs = self.generate_virtural_job_from_action(env = env, a_dict = action_dict, job_i=job_i)
